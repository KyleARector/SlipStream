...
static void test_raster_header_only(void)
{
    uint8_t buf[16];
    const uint8_t expected[] = {
        ESC, AT,
    ...
    size_t written = escpos_format_raster_header(1, 2, buf, sizeof(buf));

    TEST_ASSERT_EQUAL_UINT(sizeof(expected), written);
    TEST_ASSERT_EQUAL_UINT8_ARRAY(expected, buf, sizeof(expected));
}

static void test_raster_header_width_high_byte_encoding(void)
{
    uint8_t buf[16];
    size_t written = escpos_format_raster_header(300, 1, buf, sizeof(buf));

    TEST_ASSERT_EQUAL_UINT(10, written);
    TEST_ASSERT_EQUAL_UINT8(0x2C, buf[6]);
    TEST_ASSERT_EQUAL_UINT8(0x01, buf[7]);
}

static void test_raster_header_zero_width_rejected(void)
{
    uint8_t buf[16];
    TEST_ASSERT_EQUAL_UINT(0, escpos_format_raster_header(0, 1, buf, sizeof(buf)));
}

static void test_raster_header_zero_height_rejected(void)
{
    uint8_t buf[16];
    TEST_ASSERT_EQUAL_UINT(0, escpos_format_raster_header(1, 0, buf, sizeof(buf)));
}

static void test_raster_header_width_too_large_rejected(void)
{
    uint8_t buf[16];
    TEST_ASSERT_EQUAL_UINT(0, escpos_format_raster_header(0x10000, 1, buf, sizeof(buf)));
}

static void test_raster_header_height_too_large_rejected(void)
{
    uint8_t buf[16];
    TEST_ASSERT_EQUAL_UINT(0, escpos_format_raster_header(1, 0x10000, buf, sizeof(buf)));
}

static void test_raster_header_null_out_buf_rejected(void)
{
    TEST_ASSERT_EQUAL_UINT(0, escpos_format_raster_header(1, 1, NULL, 16));
}

static void test_raster_header_buffer_too_small_rejected(void)
{
    uint8_t buf[9];
    uint8_t sentinel[9];
    memset(sentinel, 0xAA, sizeof(sentinel));
    ...
    /* Header alone needs ESCPOS_CMD_INIT_LEN(2) + ESCPOS_CMD_RASTER_HEADER_LEN(8) = 10 bytes; buf only holds 9. */
    size_t written = escpos_format_raster_header(1, 1, buf, sizeof(buf));

    TEST_ASSERT_EQUAL_UINT(0, written);
    TEST_ASSERT_EQUAL_UINT8_ARRAY(sentinel, buf, sizeof(buf));
}

static void test_raster_header_buffer_exact_fit_succeeds(void)
{
    uint8_t buf[10];
    size_t written = escpos_format_raster_header(1, 1, buf, sizeof(buf));
    TEST_ASSERT_EQUAL_UINT(10, written);
...
int main(void)
{
    ...
    RUN_TEST(test_raster_header_only);
    RUN_TEST(test_raster_header_width_high_byte_encoding);
    RUN_TEST(test_raster_header_zero_width_rejected);
    RUN_TEST(test_raster_header_zero_height_rejected);
    RUN_TEST(test_raster_header_width_too_large_rejected);
    RUN_TEST(test_raster_header_height_too_large_rejected);
    RUN_TEST(test_raster_header_null_out_buf_rejected);
    RUN_TEST(test_raster_header_buffer_too_small_rejected);
    RUN_TEST(test_raster_header_buffer_exact_fit_succeeds);
